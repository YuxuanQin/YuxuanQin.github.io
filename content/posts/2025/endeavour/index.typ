...
当然，windows 至少让我玩到了 Sheepy: A shrot adventure。 \#\# 安装
endeavour OS 本次重装 linux 依然选择神中神的 endeavour
OS。上官网逛了一圈，发现它的含金量还在上升🤣：

- 社区支持的
  #link("https://github.com/EndeavourOS-Community-Editions/sway")[Sway 版本]自动配置了
  sway、waybar、剪切板（clipphist）、托盘图标。
...
+ 下载 ventoy，自不必多说；
+ 在 windows
  内分盘，无须赋予盘符，即：选原有盘，右键压缩卷，得到新卷即可；
...
+ 选择题：
  - 如果能访问
    Github，一切按#link("https://github.com/EndeavourOS-Community-Editions/sway")[计划]进行；
  - 如果不能访问 Github：
    - 直接无桌面离线安装；
    - 进入 tty
      后联网，克隆#link("https://codeberg.org/fup6m3vj0/endeavours-os-sway-edition")[我的备份仓库]后，进入此仓库文件夹内执行
      `sway-install.sh`；

=== 安装后
<安装后>
+ 如果所有字体都变得很大，执行
  `wlr-randr --output 显示器名字 --scale 缩放倍率` 可作相应调整，其中
  `显示器名字` 可通过执行 `wlr-randr` 获取，一般输出内容第一行就是；
+ 如果时间不正常，执行 `timedatectl set-timezone Asia/Shanghai`
  可调整至正常；
...
+ 改镜像不题；
+ `pacman -Syyu` 不题；
+ 在 `/etc/pacman.conf` 内加入 `archlinuxcn` 链接，详见
  #link("https://mirrors.tuna.tsinghua.edu.cn/help/archlinuxcn/")[tuna 指导]；
...
== 最后的注记
<最后的注记>
还有许多东西没好好讲解，譬如 `yay` 和 `pacman`，当然 arch
本身的配置也确实非常丰富，不是一篇文章能讲完的，何况网上已经有那么多好文章。
